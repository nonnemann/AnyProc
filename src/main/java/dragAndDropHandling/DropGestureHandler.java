...
import main.App;
import transferHandling.PanelDataFlavor;
...
public class DropGestureHandler implements DropTargetListener, Serializable
{
    ...
    public void drop(DropTargetDropEvent dropEvent)
    {
        if (dropEvent.isDataFlavorSupported(PanelDataFlavor.SHARED_INSTANCE))
        {
            try
            {
                ...
                Component destination = dropTargetContext.getComponent();
                
                // What to do when it is dropped?
                if(isComponentTimeSlot(destination))
                {
                    App.globalGraph.addObjectToExistingTimeslot(sourceObject, destination);
                }
                else
                {
    ...
    private boolean isComponentTimeSlot(Component component)
    {
