...
public class ConnectionSlot
{
	...
	public void generateExchangeProcesses(boolean forward)
	{
		...
		for(ConnectionSettings node : _connectionNodes.keySet())
		{
			...
			{
				SourceInSlot originTool = getTool(node, forward);
				SourceInSlot destinationTool = getTool(node, !forward);
	...
	private SourceInSlot getTool(ConnectionSettings node, boolean forward)
	{
